...
async function navigateToDeviceWebPage(page) {
    try {
        console.log('Accediendo a la WEB del dispositivo...Prueba');
...
async function iterateChannels(mainFrame, finalPath, page){
    ...
    const options = await mainFrame.$$('select#adm_bandwidth option');

    for (const option of options) {
        const value = await option.evaluate(el => el.value); // Obtiene el valor actual
        ...
        console.log("Cantidad de canales disponibles:", channelCount);
            ...
            console.log("Seleccionando configuración automática...");
